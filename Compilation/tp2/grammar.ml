...
let kleene : ('a -> 'a -> bool) -> ('a -> 'a) -> 'a -> 'a = 
 fun order f bot -> 
 let rec rec_kleene v = 
  let v' = f v in
  if not (order v v')
  then failwith "Kleene: error the function should be monotonic"
  else if order v' v
  then v' (* fixpoint *) 
  else rec_kleene v' in
 rec_kleene bot

let kleene_fun g cmp f bot = 

 let f_map mp = 
  VNMap.fold 
   (fun vn vl acc -> 
    let vl' = f (fun vn -> VNMap.find vn mp) vn in
    VNMap.add vn vl' acc) mp VNMap.empty in

 let order m m' = 
  VNMap.fold 
   (fun vn vl acc -> 
    let vl' = VNMap.find vn m' in
    cmp vl vl' && acc) m true in 

 let bot =  VNSet.fold (fun vn acc -> VNMap.add vn bot acc) g.non_terminals VNMap.empty in

 let res = kleene order f_map bot in

 (fun vn -> VNMap.find vn res)
