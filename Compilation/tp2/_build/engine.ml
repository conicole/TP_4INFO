...
let parse : grammar -> ('ulex -> vt)  ->  'ulex list  -> 'ulex parse_tree = 
  fun g terminal_of_ulex input ->
    let null = null g in
    let first = first g null in
    let follow = follow g null first in
	...
	let children, input' = 
	  parse_vlist (match deriv g null first follow vn (terminal_of_ulex vt) with None -> failwith "No derivation" | Some vlist -> vlist) input 
	in 
